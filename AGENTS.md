...
- Default to the lowest-credit production path for Bomi social ad tests: generate one `lite` square master in Moda, then create landscape/portrait derivatives locally with padding or resizing when the message and layout remain readable. Only generate additional Moda aspect ratios, higher model tiers, or extra variants when the user explicitly approves the extra credit use or the single-master output fails visual/file verification.
